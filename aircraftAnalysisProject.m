...
V = 0:1:800;
rho = (2.3679*10^-3):-0.00000203712:(7.3820*10^-4);
Cl = W./(0.5.*rho.*V.^2.*S);
Cd = Cd0 + (Cl.^2)./(pi*e*AR);
TSFC = (3000/11.06)/Ta20k;
ct = TSFC/3600;
R =  @(rho,V) (2*sqrt(2./(rho.*S))*(1/ct).*((sqrt(Cl))./Cd)*(sqrt(18000)-sqrt(15000)));
FinalRanges = R(rho,V);
rhomin = min(rho)
VhighRange = max(R(FinalRanges,V))
MaxRange = max(FinalRanges);
%% Specified Plots
