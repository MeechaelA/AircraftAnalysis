...
figure(6)
fplot(Tr15k, [150 900]), title('Thrust Required vs Velocity at 15,000 [ft]')
xlabel('Velocity [ft/s]'), ylabel('Thrust [lb_f]'), xlim([0 900]), ylim([0 6000]), grid on, hold on
[velAtMinTr, minTr] = fminbnd(Tr15k, 150, 900);
plot(velAtMinTr, minTr, 'r*')
legend('Thrust Required', 'Mininum thrust required for 15,000 [ft]', 'Location', 'southeast'), hold off

ClCdMaxEndurance15k = (W/(0.5*rho15k*velAtMinTr^2*S))/(Cd0 + ((W/(0.5*rho15k*velAtMinTr^2*S))^2/(pi*e*AR)));
...
% Figure 7 - Plots two previously determined maxR/Cs and then linearly
